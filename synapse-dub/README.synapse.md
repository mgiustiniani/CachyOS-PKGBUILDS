...
2. transcription with the official Arch `python-openai-whisper` package, or optional timestamp-capable XDNA2 Whisper with ROCm fallback;
...
The default ASR backend remains OpenAI Whisper on ROCm. Install
`synapse-whisper-xdna` and select it per workflow:

```sh
synapse-dub prepare video.mp4 --source-language it --asr-backend xdna
```

Alternatively set `ASR_BACKEND=xdna` in `/etc/synapse/dub/config.conf`.
Timestamp segments generated by the NPU decoder feed the existing diarization,
speaker mapping, translation, and dubbing stages. If XDNA is unavailable, the
workflow falls back to the existing ROCm backend.

Install the optional `synapse-whisperx-rocm` product to add ROCm word alignment
without replacing XDNA transcription:

```sh
synapse-dub prepare video.mp4 --asr-backend xdna --whisperx
```

`--whisperx-diarize` additionally enables pyannote speaker diarization and
requires explicit acceptance of the selected model terms.
