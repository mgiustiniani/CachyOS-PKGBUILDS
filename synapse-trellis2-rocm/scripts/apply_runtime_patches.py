...
def patch_birefnet():
    ...
    if 'import os\n' not in s.split('\n')[:8]:
        s = s.replace('from typing import *\n', 'from typing import *\nimport os\n')
    old_init = '''class BiRefNet:\n    def __init__(self, model_name: str = "ZhengPeng7/BiRefNet"):\n        self.model = AutoModelForImageSegmentation.from_pretrained(\n            model_name, trust_remote_code=True\n        )\n'''
    new_init = '''class BiRefNet:\n    def __init__(self, model_name: str = "ZhengPeng7/BiRefNet"):\n        model_name = os.environ.get("REMBG_MODEL_PATH", os.environ.get("REMBG_MODEL_NAME", model_name))\n        self.model_name = model_name\n        self.model = AutoModelForImageSegmentation.from_pretrained(\n            model_name, trust_remote_code=True\n        )\n'''
    if old_init in s:
        s = s.replace(old_init, new_init)
    elif 'REMBG_MODEL_NAME' not in s:
